...
var LaiZifindType = {
    ...
    GetAllLaiZi(cards, tempCard) {//取得所有賴子
        ...
        tempCard.splice(location, num);//刪除原牌中的癩子
        return Laizis;
    },
            ...
            for (var i = 1; i <= 13; i++) {
                for (var j = 1; j <= 13; j++) {
                    ...
                    for (var k = 0; k < Res.length; k++) {
                        if (Res[k][0].showTxt == item[0].showTxt && Res[k][1].showTxt == item[1].showTxt) {
                            newitem = false;
                            break;
                        }
                    }
    ...
    TryLaiZis(tempCard, tryNum) {
        ...
        for (var i = 0; i < tryNum.length; i++) {//一一帶入嘗試
            ...
            tryCard.sort(CardUtil.gradeUp);//重新排列
